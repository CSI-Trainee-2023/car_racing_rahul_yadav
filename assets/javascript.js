let startTime, lapStartTime, currentTime, elapsedTime = 0;
 let isRunning = false; //stopwatch is not running.
let lapCount = 1;
...
function startStopwatch() {
  if (isRunning) {
    ...
    if (elapsedTime === 0) {
      startTime = Date.now();  //sets the `startTime to the current time
    } else {
...
function updateStopwatch() {
  if (isRunning) {
    currentTime = Date.now() - startTime;  //date.now() measure the current time
    updateDisplay();  
    // requestAnimationFrame(updateStopwatch);
  }
}
